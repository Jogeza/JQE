...
from tools.paper_runtime import evaluate_production_decision
...
def _record(session_id: str, cycle: int, observation: ClosedMarketObservation, heartbeat: Any, facts: dict[str, object], began: datetime, runtime: Any) -> PaperObservationRecord:
    closed = runtime.engine.closes[-1] if runtime.engine.closes else None
    raw_direction = str(facts.get("signal_direction", "NO_TRADE"))
...
async def run_campaign(*, symbol: str, timeframe: Timeframe, max_observations: int, output: Path, diagnostics_path: Path | None = None, session_id: str | None = None, resume: bool = False, quiet: bool = False) -> dict[str, Any]:
    ...
    try:
        ...
        async def decide(window: list[ClosedMarketObservation]):
            nonlocal facts
            decision, facts = await evaluate_production_decision(window)
            return decision
        ...
        summary = {"schema_version": 1, "session": {
...
def main() -> int:
    ...
    summary = asyncio.run(run_campaign(symbol=args.symbol, timeframe=args.timeframe, max_observations=args.max_observations, output=args.output, diagnostics_path=args.diagnostics_path, session_id=args.session_id, resume=args.resume, quiet=args.quiet))
